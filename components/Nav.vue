<template>
    <header
        ...
        <ul class="hidden desktop:flex gap-5">
            <li v-for="linkItem in links" @click="linkScroll(linkItem.link)" :key="linkItem.id"
        ...
        <div @click="isShowNav = !isShowNav" class="cursor-pointer pr-4 z-10 text-gray-100 desktop:hidden">
        ...
        <ul 
            ...
            <li v-for="linkItem in links" @click="linkScroll(linkItem.link)" :key="linkItem.id" class="px-4 cursor-pointer py-6 text-4xl opacity-100">
                {{ linkItem.link }}
            </li>
        </ul>
    </header>
